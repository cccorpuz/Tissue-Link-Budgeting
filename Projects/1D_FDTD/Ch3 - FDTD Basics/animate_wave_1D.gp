...
stats '<awk -F, "{for(i=1;i<=NF;i++) print $i}" sim_output.csv' using 1 nooutput
...
do for [i=0:STATS_records-1] {
    plot '<awk -F, "NR==' . (i+1) . '{for(j=1;j<=NF;j++) print j,$j}" sim_output.csv' using 1:2 with lines title sprintf("Frame %d", i+1)
}
set output